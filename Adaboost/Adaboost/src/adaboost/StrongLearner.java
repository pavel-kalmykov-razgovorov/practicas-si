...
class StrongLearner {
     ...
     */
    StrongLearner(ArrayList<Cara> faces, ArrayList<Cara> testFaces, int numWeakClassifiers, int numHyperplanes) {
        ...
        System.out.println("Iteration\t%Matches(Learn)\tFacesError(Learn)\tNo-FacesError(Learn)\t%Matches(Test)\tFacesError(test)\tNo-FacesError(Test)");
        for (int t = 0; t < numWeakClassifiers; t++) {
            ...
            if(stats[0] == faces.size()) break; //Si las acierto todas termino
        }
    }
